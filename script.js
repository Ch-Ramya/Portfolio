...
const systemPreferenceTheme = window.matchMedia("(prefers-color-scheme: light)");
...
const experienceTimelineContainer = document.querySelector(".experience-timeline");

const roles = ["Frontend Developer.", "Web Enthusiast.", "UI Engineer."];
const buttonCtaText = {"light" : {"label":"Toggle dark mode", "icon" : "fa-solid fa-moon"}, "dark" : {"label":"Toggle light mode", "icon" : "fa-solid fa-sun"}};
const skills = [
...
function generateExperienceTimeline(data) {
    ...
    data.forEach((experience, index) => {
        ...
        companyContainer.classList.add("company-container", index % 2 === 0 ? "left-container" : "right-container");
        ...
        span.classList.add(index % 2 === 0 ? "left-container-arrow" : "right-container-arrow");
...
let currentTheme = getInitialThemePreference(localStorageTheme, systemPreferenceTheme);
